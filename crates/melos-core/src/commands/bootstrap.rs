...
pub fn generate_pubspec_overrides(
    ...
    workspace_root: &Path,
    melos_overrides: &HashMap<String, yaml_serde::Value>,
    ...
    for pkg in packages {
        ...
        if local_deps.is_empty() && melos_overrides.is_empty() {
        ...
        let content = build_pubspec_overrides_content(&local_deps, &pkg.path, melos_overrides)?;
...
///
/// Workspace sibling overrides are written first (sorted by name), then
/// entries from `melos_overrides.yaml` are appended. Sibling overrides take
/// precedence over `melos_overrides` entries for the same package name.
pub fn build_pubspec_overrides_content(
    local_deps: &[&Package],
    pkg_path: &Path,
    melos_overrides: &HashMap<String, yaml_serde::Value>,
) -> Result<String> {
    ...
    // Collect sibling package names so melos_overrides don't duplicate them
    let sibling_names: HashSet<&str> = local_deps.iter().map(|p| p.name.as_str()).collect();
    ...
    // that are already covered by workspace sibling overrides.
    if !melos_overrides.is_empty() {
        let mut sorted_overrides: Vec<_> = melos_overrides
            .iter()
            .filter(|(name, _)| !sibling_names.contains(name.as_str()))
            .collect();
        sorted_overrides.sort_by_key(|(name, _)| name.as_str());
...
// ---------------------------------------------------------------------------
// Git dependency ref comparison (Melos v7.0.0-dev.3, #659)
// ---------------------------------------------------------------------------

/// Per-package snapshot of git dependencies: `dep_name -> (url, ref)`.
pub type GitDepsSnapshot = HashMap<String, HashMap<String, (String, String)>>;

/// Extract `(dep_name, url, ref)` triples from a package's pubspec.yaml.
///
/// Scans `dependencies` and `dev_dependencies` for entries of the form:
/// ```yaml
/// dep:
///   git:
///     url: https://...
...
pub fn extract_git_deps(pkg: &Package) -> Vec<(String, String, String)> {
    ...
    for section in ["dependencies", "dev_dependencies"] {
        if let Some(yaml_serde::Value::Mapping(deps)) = parsed.get(section) {
            for (key, value) in deps {
                let dep_name = match key {
                    yaml_serde::Value::String(s) => s.clone(),
                    _ => continue,
                };
                if let yaml_serde::Value::Mapping(dep_map) = value {
                    let git_key = yaml_serde::Value::String("git".to_string());
                    if let Some(yaml_serde::Value::Mapping(git_map)) = dep_map.get(&git_key) {
                        let url = git_map
                            .get(yaml_serde::Value::String("url".to_string()))
                            .and_then(|v| v.as_str())
                            .unwrap_or("")
                            .to_string();
                        let git_ref = git_map
                            .get(yaml_serde::Value::String("ref".to_string()))
                            .and_then(|v| v.as_str())
                            .unwrap_or("")
                            .to_string();
                        if !url.is_empty() {
                            git_deps.push((dep_name, url, git_ref));
                        }
                    }
                }
            }
...
/// Snapshot file path: `<workspace_root>/.melos_tool/git_deps.json`.
fn git_deps_snapshot_path(workspace_root: &std::path::Path) -> PathBuf {
    workspace_root.join(".melos_tool").join("git_deps.json")
}
...
/// Save the current git dep snapshot.
pub fn save_git_deps_snapshot(
    workspace_root: &std::path::Path,
    snapshot: &GitDepsSnapshot,
) -> Result<()> {
    let path = git_deps_snapshot_path(workspace_root);
...
/// Compare current git deps against the saved snapshot. Returns names of
/// packages whose git deps have changed (requiring cache invalidation).
pub fn detect_git_dep_changes(
    packages: &[Package],
    workspace_root: &std::path::Path,
) -> (Vec<String>, GitDepsSnapshot) {
    let old_snapshot = load_git_deps_snapshot(workspace_root);
    let mut new_snapshot: GitDepsSnapshot = HashMap::new();
    let mut changed_pkgs = Vec::new();

    for pkg in packages {
        let git_deps = extract_git_deps(pkg);
        if git_deps.is_empty() {
            continue;
        }
        let current: HashMap<String, (String, String)> = git_deps
            .into_iter()
            .map(|(name, url, r)| (name, (url, r)))
            .collect();

        if let Some(old) = old_snapshot.get(&pkg.name) {
            if old != &current {
                changed_pkgs.push(pkg.name.clone());
            }
        } else {
            // New entry — treat as changed on first bootstrap
            changed_pkgs.push(pkg.name.clone());
        }

        new_snapshot.insert(pkg.name.clone(), current);
    }

    (changed_pkgs, new_snapshot)
}

/// Invalidate Dart's package resolution cache for a package by removing
/// `.dart_tool/package_config.json`.
pub fn invalidate_package_cache(pkg: &Package) -> Result<()> {
    let cache_file = pkg.path.join(".dart_tool").join("package_config.json");
    if cache_file.exists() {
        std::fs::remove_file(&cache_file).with_context(|| {
            format!(
                "Failed to remove {} for git dep re-resolution",
                cache_file.display()
            )
        })?;
    }
...
mod tests {
    ...
    fn test_build_pubspec_overrides_content() {
        ...
        let content = build_pubspec_overrides_content(&deps, &app_path, &HashMap::new()).unwrap();
    ...
    // -- melos_overrides.yaml merge tests (Batch 56, Melos v3.0.0 #410) --

    #[test]
    fn test_build_pubspec_overrides_with_melos_overrides() {
        let core = make_package("core", "/workspace/packages/core", vec![]);
        let app_path = PathBuf::from("/workspace/packages/app");

        let mut overrides = HashMap::new();
        overrides.insert(
            "external_pkg".to_string(),
            yaml_serde::Value::Mapping({
                let mut m = yaml_serde::Mapping::new();
                m.insert(
                    yaml_serde::Value::String("path".to_string()),
                    yaml_serde::Value::String("../../external/external_pkg".to_string()),
                );
                m
            }),
        );

        let deps: Vec<&Package> = vec![&core];
        let content = build_pubspec_overrides_content(&deps, &app_path, &overrides).unwrap();

        // Sibling override present
        assert!(content.contains("  core:"), "Missing core override");
    ...
    #[test]
    fn test_melos_overrides_sibling_takes_precedence() {
        let core = make_package("core", "/workspace/packages/core", vec![]);
        let app_path = PathBuf::from("/workspace/packages/app");

        // melos_overrides also defines "core" — should be skipped (sibling wins)
        let mut overrides = HashMap::new();
        overrides.insert(
            "core".to_string(),
            yaml_serde::Value::Mapping({
                ...
                m.insert(
                    yaml_serde::Value::String("path".to_string()),
                    yaml_serde::Value::String("/other/core".to_string()),
                );
                m
            }),
        );
        ...
        // Only one "core:" entry (the sibling), not the melos_overrides one
    ...
    #[test]
    fn test_generate_pubspec_overrides_with_only_melos_overrides() {
        let dir = tempfile::tempdir().unwrap();
        let app_dir = dir.path().join("packages").join("app");
        std::fs::create_dir_all(&app_dir).unwrap();
        // app has no sibling dependencies
        let app = make_package("app", &app_dir.to_string_lossy(), vec![]);
        ...
        overrides.insert(
            ...
            yaml_serde::Value::Mapping({
                ...
                m.insert(
                    yaml_serde::Value::String("path".to_string()),
                    yaml_serde::Value::String("../../external/ext".to_string()),
                );
                m
            }),
        );

        let result = generate_pubspec_overrides(&[app], &[], &[], dir.path(), &overrides);
        assert!(result.is_ok());
    ...
    fn test_extract_git_deps_none() {
        ...
        .unwrap();
        let pkg = make_package("app", &dir.path().to_string_lossy(), vec!["http"]);
        let git_deps = extract_git_deps(&pkg);
        assert!(git_deps.is_empty());
    }

    #[test]
    fn test_extract_git_deps_found() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::write(
            dir.path().join("pubspec.yaml"),
        ...
        .unwrap();
        let pkg = make_package("app", &dir.path().to_string_lossy(), vec!["my_lib"]);
        let git_deps = extract_git_deps(&pkg);
        assert_eq!(git_deps.len(), 1);
        assert_eq!(git_deps[0].0, "my_lib");
        assert_eq!(git_deps[0].1, "https://github.com/org/my_lib.git");
        assert_eq!(git_deps[0].2, "v2.0.0");
    }

    #[test]
    fn test_detect_git_dep_changes_new_entry() {
        let ws_dir = tempfile::tempdir().unwrap();
        let pkg_dir = ws_dir.path().join("packages").join("app");
        ...
        let (changed, snapshot) = detect_git_dep_changes(&[pkg], ws_dir.path());
        // First time — treated as changed
        assert_eq!(changed, vec!["app"]);
        assert!(snapshot.contains_key("app"));
    }

    #[test]
    fn test_detect_git_dep_changes_unchanged() {
        let ws_dir = tempfile::tempdir().unwrap();
        let pkg_dir = ws_dir.path().join("packages").join("app");
        std::fs::create_dir_all(&pkg_dir).unwrap();
        std::fs::write(
    ...
    #[test]
    fn test_detect_git_dep_changes_ref_changed() {
        let ws_dir = tempfile::tempdir().unwrap();
        let pkg_dir = ws_dir.path().join("packages").join("app");
        std::fs::create_dir_all(&pkg_dir).unwrap();
        std::fs::write(
        ...
        let (_, snapshot) = detect_git_dep_changes(&[pkg], ws_dir.path());
        save_git_deps_snapshot(ws_dir.path(), &snapshot).unwrap();

        // Change ref in pubspec
        std::fs::write(
            pkg_dir.join("pubspec.yaml"),
            "name: app\nversion: 1.0.0\ndependencies:\n  lib:\n    git:\n      url: https://example.com/lib.git\n      ref: v2.0.0\n",
        )
        .unwrap();
        let pkg2 = make_package("app", &pkg_dir.to_string_lossy(), vec!["lib"]);

        let (changed, _) = detect_git_dep_changes(&[pkg2], ws_dir.path());
        assert_eq!(changed, vec!["app"]);
    }

    #[test]
    fn test_invalidate_package_cache() {
        let dir = tempfile::tempdir().unwrap();
        let dart_tool = dir.path().join(".dart_tool");
        std::fs::create_dir_all(&dart_tool).unwrap();
        let config_file = dart_tool.join("package_config.json");
    ...
    fn test_generate_pubspec_overrides_no_extra_paths() {
        ...
        let result = generate_pubspec_overrides(&[app], &[core], &[], dir.path(), &HashMap::new());
        assert!(result.is_ok());
